...
export function DataGrid({ reserveRatio, baseReserves, oraclePrice, onInspectProtocol }: DataGridProps) {
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 sm:gap-4 w-full">
      ...
      <MetricCard
        label="RESERVE RATIO"
        value={reserveRatio === 0 ? 'NO DATA' : `${displayRatio.toFixed(2)}%`}
        onInspect={onInspectProtocol}
        hasInspector={true}
        isNoData={reserveRatio === 0}
      />

      {/* Base Reserves Metric */}
      <MetricCard
        label="BASE RESERVES"
        value={baseReserves === 0 ? 'NO DATA' : `${formatReserves(displayReserves)} ERG`}
        isNoData={baseReserves === 0}
      />
...
function MetricCard({ label, value, onInspect, hasInspector, isNoData }: MetricCardProps) {
  return (
    <motion.div
    ...
    >
      ...
      <div className="flex flex-col justify-between h-full">
        ...
        <div className={`text-base sm:text-2xl lg:text-4xl font-mono font-bold break-all transition-all ${
          isNoData ? 'text-amber-500/60' : 'text-[#39FF14] group-hover:text-bloom-green'
        }`}>
          {value}
        </div>
      </div>
