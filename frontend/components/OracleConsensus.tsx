...
export default function OracleConsensus() {
  ...
  // Fetch real ERG price from our prices API
  ...
  // Generate oracle node data based on real price
  const generateNodes = useCallback((basePrice: number) => {
    // Simulate 6 oracle nodes with slight variance from real price
    const nodeIds = [
      '4a3f...2b9c',
      '8d7e...5c1a', 
      '1b9f...3e4d',
    ...
    const generatedNodes: OracleNode[] = nodeIds.map((id, index) => {
      // Each node has slight variance (±1% from real price)
      const variance = (Math.random() * 0.02 - 0.01) * basePrice;
      return {
        id,
    ...
    const mean = generatedNodes.reduce((sum, node) => sum + node.price, 0) / generatedNodes.length;
    
    // Determine sync status based on deviation from mean
    const processedNodes = generatedNodes.map(node => {
      const deviation = Math.abs(node.price - mean) / mean;
      return {
        ...node,
        status: (deviation > 0.01 ? 'desync' : 'synced') as 'synced' | 'desync'
      };
    });

    return { nodes: processedNodes, mean };
  }, []);

  useEffect(() => {
    // Get real price from API only - no fallback
    const basePrice = priceData?.prices?.ERG?.price;
    
    if (!basePrice) {
      return; // Don't update nodes without real data
    }
    
    const { nodes: initialNodes, mean } = generateNodes(basePrice);
    setNodes(initialNodes);
    setMeanPrice(mean);

    // Update periodically with slight variance
    const interval = setInterval(() => {
      const currentPrice = priceData?.prices?.ERG?.price || basePrice;
      const { nodes: updatedNodes, mean: newMean } = generateNodes(currentPrice);
      setNodes(updatedNodes);
      setMeanPrice(newMean);
    }, 8000); // Update every 8 seconds

    return () => clearInterval(interval);
  }, [priceData, generateNodes]);
