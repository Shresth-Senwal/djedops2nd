...
export default function StressTest() {
  ...
  // Fetch real ERG price and Djed protocol state
  const { data: priceData, error: priceError } = useSWR('/api/prices?symbols=ERG', fetcher, { refreshInterval: 30000 });
  const { data: djedState, error: djedError } = useSWR('/api/djed?endpoint=djed/state', fetcher, { refreshInterval: 30000 });
  
  // Use real data only - no fallbacks
  const BASE_PRICE = priceData?.prices?.ERG?.price;
  const BASE_RATIO = djedState?.data?.reserveRatio;
  
  // ALL HOOKS MUST BE CALLED BEFORE ANY EARLY RETURNS
  useEffect(() => {
    // Calculate projected ratio based on price change
    // Reserve Ratio = (baseReserves * ergPrice / stablecoinSupply) * 100
    // When price changes by X%, ratio changes by X%
    if (BASE_RATIO) {
      const calculatedRatio = BASE_RATIO * (1 + sliderPercent / 100);
      setProjectedRatio(calculatedRatio);
    }
  }, [sliderPercent, BASE_RATIO]);
  
  // NOW we can do early returns after all hooks are called
  // Show error state if data unavailable
  if (priceError || djedError) {
    return (
      <div className="bg-red-900/20 border border-red-500/50 rounded-lg p-6">
        <div className="text-red-400 font-mono text-sm">
          ⚠️ Unable to load real-time data from API
          {priceError && <div className="mt-2">Price API: {priceError.message}</div>}
          {djedError && <div className="mt-2">Djed API: {djedError.message}</div>}
        </div>
      </div>
    );
  }
  ...
  if (!BASE_PRICE || !BASE_RATIO || BASE_RATIO === 0) {
    return (
      <div className="bg-black/60 border border-[#39FF14]/20 rounded-lg p-6">
        <div className="text-[#39FF14]/60 font-mono text-sm animate-pulse">
          Loading Ergo blockchain data...
        </div>
      </div>
    );
  }
